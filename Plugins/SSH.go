...
func SshScan(info *Common.HostInfo) error {
	...
	go func() {
		...
		if Common.SshKeyPath != "" {
			Common.LogDebug("使用SSH密钥认证: %s", Common.SshKeyPath)
			...
			for _, user := range Common.Userdict["ssh"] {
				...
				case <-globalCtx.Done():
					...
					return
				default:
					Common.LogDebug("尝试使用密钥认证用户: %s", user)
					...
					if success {
						...
						Common.LogDebug("密钥认证失败: %s, 错误: %v", user, err)
					}
				}
			}
		...
		Common.LogDebug("开始尝试用户名密码组合 (总用户数: %d, 总密码数: %d, 总组合数: %d)",
	...
	case <-globalCtx.Done():
		Common.LogDebug("扫描 %s 全局超时", target)
		return fmt.Errorf("全局超时，扫描未完成")
	}

	Common.LogDebug("扫描完成，未发现有效凭据")
	return nil
}
	...
	go func() {
		for i, cred := range credentials {
			...
			case <-scanCtx.Done():
				break
			default:
				Common.LogDebug("[%d/%d] 尝试: %s:%s", i+1, len(credentials), cred.Username, cred.Password)
				workChan <- cred
			}
		}
		close(workChan)
...
func trySshCredential(info *Common.HostInfo, credential SshCredential, timeout int64, maxRetries int) *SshScanResult {
	...
	for retry := 0; retry < maxRetries; retry++ {
		if retry > 0 {
			Common.LogDebug("第%d次重试: %s:%s", retry+1, credential.Username, credential.Password)
...
func logAndSaveSuccess(info *Common.HostInfo, target string, result *SshScanResult) {
	...
	Common.LogSuccess("%s", successMsg)
